...
\begin{exmp}
	...
	\[Y_i \sim Bin(1,p(X_i)). \]
	Цель: оценить функцию $p: \left[0, \infty \right) \rightarrow [0, 1]$. Упрощение: "параметрическая модель". Типичный пример: \[p(x)=1-e^{-\beta x}, \quad \beta>0 .\]
	Следовательно, оценка функции $p(x)$ эквивалентна оценке параметра $\beta$.
\end{exmp}
...
\begin{defn} \
	\begin{enumerate}
		\item Оценка $g^* \in \mathcal{K}$ называется \textbf{\textit{допустимой}}, если не существует такой оценки $g \in \mathcal{K}$, что
		...
		\item Класс $\tilde{\mathcal{K}} \subset \mathcal{K}$ называется \textbf{\textit{полным}}, если для любой оценки $g \in \mathcal{K} \backslash \tilde{\mathcal{K}}$ существует такая оценка $\tilde{g} \in \tilde{\mathcal{K}}$, что
...
\begin{proof}
	...
	и $Y=(Y_1, \dots, Y_n)^T$. Выберем такую ортогональную матрицу $A$, что её последняя строчка:
...
\begin{defn} \
	\begin{enumerate}
		...
		\item Если оценка $g$ такая, что неравенство \eqref{eq2.2} превращается в равенство, то такая оценка называется \textbf{\textit{эффективной}} для $\gamma(\vartheta)=\ME_\vartheta[g(X)]$.
	\end{enumerate}
\end{defn}
...
\begin{defn} \
	\begin{enumerate}
		\item Семейство $\mathcal{P}= \{ P_\vartheta \mid \vartheta \in \Theta \}$ называется \textbf{\textit{экспоненциальным семейством}}, если существуют такие вещественнозначные $c, Q_1, \dots, Q_k : \Theta \rightarrow \MR$ и измеримые функции $h, T_1, \dots, T_k: \mathcal{X} \rightarrow \MR$, что $\mu$-плотность $P_\vartheta$:
